...
# Modern Monochrome/Grayscale color palette theme
st.markdown("""
<style>
    ...
    /* Headers - White with subtle glow */
    h1, h2, h3 {
...
def create_download_link(fig, filename):
    ...
    try:
        ...
        href = f'<a href="data:image/png;base64,{b64}" download="{filename}" style="color: #ffffff; text-decoration: none; font-weight: 600;">📥 Download Chart as PNG</a>'
        return href
    except:
        return ""
...
def display_real_time_view(config, refresh_interval):
    ...
    if display_data is not None and not display_data.empty:
        ...
        fig.update_layout(
            ...
            xaxis=dict(showgrid=True, gridcolor='#404040', title='Time'),
            yaxis=dict(showgrid=True, gridcolor='#404040', title='Price (USD)'),
            legend=dict(
                orientation="h",
                yanchor="bottom",
                y=1.02,
        ...
        with st.spinner(f"Analyzing {selected_display} price trends..."):
            ...
            if df is not None and not df.empty:
                ...
                fig.update_layout(
                    ...
                    hovermode='x unified',
                    legend=dict(bgcolor='rgba(26, 26, 26, 0.8)')
                )
